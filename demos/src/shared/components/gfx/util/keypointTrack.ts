import { applyController, HANDLED_CTRL_KINDS, type CtrlState, type KpController, type KpCtrlKind } from './controllers';
import { argbLerp } from './argb';
import { lerp } from './math';
import type { AutoangelModule } from '../../../../types/autoangel';

type ParseGfxResult = ReturnType<AutoangelModule['parseGfx']>;
type GfxElement = ParseGfxResult['elements'][number];
...
export interface Track {
  ...
  readonly controllers: readonly (readonly KpController[])[];
...
export interface Sample {
  ...
  normalized: number;
  /** Active keypoint segment index (0..keypoints.length-1). */
  kpIndex: number;
}
...
const EMPTY_TRACK: Track = {
  ...
  unhandledCount: 0, unhandledKinds: new Set(),
};
...
export function buildTrack(kps: KeyPointSet | undefined): Track {
  ...
  for (const list of controllers) {
    for (const ctrl of list) {
      if (!HANDLED_CTRL_KINDS.has(ctrl.body.kind)) {
        unhandledCount++;
        unhandledKinds.add(ctrl.body.kind);
      }
    }
  }
  return {
...
export function sampleTrack(track: Track, tMs: number): Sample {
  if (track.colors.length === 0) {
    return { color: 0, position: [0, 0, 0], scale: 1, direction: [0, 0, 0, 1], rad2d: 0, normalized: 0, kpIndex: 0 };
  }
  if (!track.loopable) {
    return { ...baseStateAt(track, 0), normalized: 0, kpIndex: 0 };
  }
  ...
  for (let i = 0; i < last; i++) {
    ...
    if (tMs >= segStart && tMs < segEnd) {
      ...
      return {
        ...state,
        // Direction lerped as raw quat components; proper slerp is follow-up.
  ...
  return { ...baseStateAt(track, last), normalized: 1, kpIndex: last };
}
