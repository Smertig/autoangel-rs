...
describe('buildTrack', () => {
  ...
  it('records unhandled controller kinds (deferred ones)', () => {
    const t = buildTrack({
      start_time: 0,
      keypoints: [
        kp({ time_span: 100, controllers: [{ start_time: 0, end_time: -1, body: { kind: 'curve_move', calc_dir: false, vertices: [[0,0,0]] } as any }] }),
        kp({ time_span: 100 }),
      ],
    });
    expect(t.unhandledKinds.has('curve_move')).toBe(true);
