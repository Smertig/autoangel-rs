...
export function ParticlePreview({ body, element, context, expanded }: PreviewProps<'particle'>) {
  if (!expanded) return <span className={styles.thumb}>P</span>;

  const rows = buildFieldRows(body, element, context);
  const shape = body.emitter.shape.shape;

  return (
    <div className={styles.expanded}>
      <div className={styles.split}>
        <div>
          {shape === 'point' || shape === 'ellipsoid' || shape === 'cylinder'
...
function ShapePending({ shape }: { shape: string }) {
  return (
    <div className={styles.pending}>
      3D simulation for <code>{shape}</code> shape — coming later.
    </div>
  );
}
...
function ParticleSim({
  ...
  const { canvasRef, readoutRef } = useParticleCanvas(body, element, context);
  const colorMinCss = argbToCss(body.emitter.color_min);
  const colorMaxCss = argbToCss(body.emitter.color_max);
  return (
    <div className={styles.sim}>
      <div ref={canvasRef} className={styles.canvas} />
      <div className={styles.vignette} />
      <span className={`${styles.bezelTick} ${styles.top}`}>1m</span>
      <span className={`${styles.bezelTick} ${styles.bottom}`}>1m</span>
      <span className={`${styles.bezelTick} ${styles.left}`}>1m</span>
      <span className={`${styles.bezelTick} ${styles.right}`}>1m</span>
      <div ref={readoutRef} className={styles.readout} />
      <div
        className={styles.colorBar}
        style={{ background: `linear-gradient(to right, ${colorMinCss}, ${colorMaxCss})` }}
      />
    </div>
  );
}
...
function buildFieldRows(
  ...
  rows.push({
    label: 'tex_file',
    value: <PathOrText value={element.tex_file} findFile={context.findFile} onNavigate={context.onNavigateToFile} />,
  });
  rows.push({
    label: 'tex_grid',
    value: (
