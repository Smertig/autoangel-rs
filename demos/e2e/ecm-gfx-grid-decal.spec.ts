...
// on the default clip (站立) referencing gfx\grid_decal_3d.gfx — a real grid
// decal element. Proves the registry dispatches 'grid_decal_3d' through
// spawnGridDecalRuntime.
// Rebuild via: cd autoangel-py && uv run python ../demos/e2e/generate-fixtures.py
const FIXTURE_PCK = path.resolve(__dirname, 'fixtures/models/ecm_with_gfx_grid_decal_event.pck');

test('grid_decal_3d GFX spawns in ECM viewer when event fires', async ({ page }) => {
