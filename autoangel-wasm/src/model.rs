...
/// Parse an SMD (skin model data) file from bytes. Returns an `SmdModel` —
/// TypeScript types are generated by `tsify-next` from the Rust definitions
/// in `autoangel_core::model::smd`.
#[wasm_bindgen(js_name = "parseSmd")]
pub fn parse_smd(data: &[u8]) -> Result<autoangel_core::model::smd::SmdModel, JsError> {
    let ds = DataSource::from_bytes(data.to_vec());
    pollster::block_on(autoangel_core::model::smd::SmdModel::parse(&ds))
        .map_err(|e| crate::format_error(&e))
}
